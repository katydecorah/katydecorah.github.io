...
The other day I came across the Dribbble shot [Cross by Helvetic Brands](http://drbl.in/kxaI). I loved the pixels and the animation, so I started to code it out. I used a single element to create the first pixel and then used `box-shadow` to generate the rest. I started to add the animation, but I spun myself in another direction.
...
{% highlight css %}
animation: cross 1s infinite ease-in $timing;
{% endhighlight %}
...
{% highlight css %}
animation: cross 1s infinite ease-in -1s;
{% endhighlight %}
