...
const getPlaylist = () => {
  const opts = {
    url: `https://api.spotify.com/v1/users/katydecorah/playlists/${process.env.SpotifyPlaylist}?fields=name%2Cimages%2Cexternal_urls%2Ctracks.items(track(name%2Calbum(name)%2Cartists(name)))`,
    method: 'GET',
    headers: {
...
const createPost = (data) => {
  ...
  return new Promise((resolve) => {
    fs.writeFile(`_posts/playlists/${moment().format('YYYY-MM-DD')}-${data.formatted_name}.md`, contents, (err) => {
      if (err) throw err;
      resolve(data);
    });
  });
