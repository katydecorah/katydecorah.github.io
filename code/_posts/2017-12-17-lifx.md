...
The value of `bulb` is known as a [selector](https://api.developer.lifx.com/docs/selectors) in the [LIFX API](https://api.developer.lifx.com/). For example, I can pass through a single bulb ID, the keyword `all` for all my lights, or a group ID.
...
I have another script in the repo that my IoT button runs or I can run from the command line:
